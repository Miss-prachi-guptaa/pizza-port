...
router.patch('/restaurant', restaurantController.updateRestaurantProfile); // name, address, phone, logo, delivery_charge etc.
router.patch('/restaurant/status', restaurantController.toggleOpenStatus); // body: { is_open: true/false }

// -------- Menu management --------
router.get('/menu-items', menuItemController.getMyMenuItems);
router.post('/menu-items', menuItemController.createMenuItem);
router.patch('/menu-items/:id', menuItemController.updateMenuItem);         // edit fields, incl. is_available toggle
...
router.patch('/orders/:id/accept', orderController.acceptOrder); // placed -> preparing, sets accepted_at
router.patch('/orders/:id/reject', orderController.rejectOrder); // placed -> rejected, sets closed_at
